...
//对于保守的算术安全，例如(TIMER_DATE_LIMIT + TIMER_INTERVAL_LIMIT + kCFAbsoluteTimeIntervalSince1970) * 10^9 < 2^63
//const CFTimeInterval kCFAbsoluteTimeIntervalSince1970 = 978307200.0L;
#define TIMER_DATE_LIMIT                        4039289856.0//4秒
#define TIMER_INTERVAL_LIMIT                    504911232.0//0.5秒
...
static void __THE_SYSTEM_HAS_NO_PORTS_AVAILABLE__(kern_return_t ret) {
    HALT;
}
...
#pragma mark - __CFRunLoopModei结构体
...
struct __CFRunLoop {
    ...
    CFMutableSetRef _modes; //所有Modes
...
static CFMutableDictionaryRef __CFRunLoops = NULL;//保存RunLoop的字典
...
CF_EXPORT CFRunLoopRef _CFRunLoopGet0(pthread_t t) {
    ...
    //保存RunLoop的字典不存在则创建字典，并且创建主线程的RunLoop
    if (!__CFRunLoops) {
...
static CFIndex __CFRunLoopInsertionIndexInTimerArray(CFArrayRef array, CFRunLoopTimerRef rlt) {
    ...
    if (256 < cnt) {
        ...
        //最后一个Timer触发时间小于 当前要插入的Timer触发时间
        ...
        // 当前要插入的Timer触发时间 还小于第一个Timer要触发的时间
    ...
    do {
        ...
        if (testIdx < cnt) {
            ...
            if (item->_fireTSR <= rlt->_fireTSR) {//当前小于等于目标值
                idx = testIdx;
                lastTestLEQ = true;
            }
        }
...
static void __CFArmNextTimerInMode(CFRunLoopModeRef rlm, CFRunLoopRef rl) {
    uint64_t nextHardDeadline = UINT64_MAX;//硬触发时间
    uint64_t nextSoftDeadline = UINT64_MAX;//软触发时间
    ...
    if (rlm->_timers) {
        ...
        if (nextSoftDeadline < UINT64_MAX && (nextHardDeadline != rlm->_timerHardDeadline || nextSoftDeadline != rlm->_timerSoftDeadline)) {
            ...
            //nextHardDeadline 大于nextSoftDeadline 使用dispatch timer
            if (leeway > 0) {
...
static Boolean __CFRunLoopDoTimer(CFRunLoopRef rl, CFRunLoopModeRef rlm, CFRunLoopTimerRef rlt) {       /* DOES CALLOUT */
    ...
    if (__CFIsValid(rlt) && rlt->_fireTSR <= mach_absolute_time() && !__CFRunLoopTimerIsFiring(rlt) && rlt->_runLoop == rl) {
        ...
        __CFArmNextTimerInMode(rlm, rl);//计算下一次理应触发时间点
...
static Boolean __CFRunLoopDoTimers(CFRunLoopRef rl, CFRunLoopModeRef rlm, uint64_t limitTSR) {  /* DOES CALLOUT */
    ...
    for (CFIndex idx = 0, cnt = rlm->_timers ? CFArrayGetCount(rlm->_timers) : 0; idx < cnt; idx++) {
        ...
        if (__CFIsValid(rlt) && !__CFRunLoopTimerIsFiring(rlt)) {
            if (rlt->_fireTSR <= limitTSR) {//当前RunLoop中该Model对应下的Timer 理应时间小于等于 当前时间
                if (!timers) {
                    timers = CFArrayCreateMutable(kCFAllocatorSystemDefault, 0, &kCFTypeArrayCallBacks);
                }

                CFArrayAppendValue(timers, rlt); //加入处理待处理数组中
            }
        }
    }
...
static int32_t __CFRunLoopRun(CFRunLoopRef rl, CFRunLoopModeRef rlm, CFTimeInterval seconds, Boolean stopAfterHandle, CFRunLoopModeRef previousMode) {
    ...
    uint64_t startTSR = mach_absolute_time();//获取当前的纳秒时间
    ...
    if (seconds <= 0.0) { // instant timeout  直接超时
        seconds = 0.0;
        timeout_context->termTSR = 0ULL;
    }
    else if (seconds <= TIMER_INTERVAL_LIMIT) { //不超过TIMER_INTERVAL_LIMIT
    ...
    else {   // infinite timeout  不会超时
        seconds = 9999999999.0;
        timeout_context->termTSR = UINT64_MAX;
    }

    Boolean didDispatchPortLastTime = true;
    int32_t retVal = 0;//retVal只要为0，runloop就一直在当前mode下循环
 ...
 handle_msg:;
        ...
        else if (timeout_context->termTSR < mach_absolute_time()) {//超时退出
        ...
        else if (__CFRunLoopModeIsEmpty(rl, rlm, previousMode)) {//没有Timer/source/observer退出
...
//UITableView滑动时，Runloop会进行切换mode，
//由kCFRunLoopDefaultMode切换为UITrackingRunLoopMode，根据源码，切换mode实际是调用CFRunLoopRunSpecific 这个函数
...
void CFRunLoopAddTimer(CFRunLoopRef rl, CFRunLoopTimerRef rlt, CFStringRef modeName) {
    ...
    if (modeName == kCFRunLoopCommonModes) {
        ...
        if (NULL == rl->_commonModeItems) {//没有则创建
            rl->_commonModeItems = CFSetCreateMutable(kCFAllocatorSystemDefault, 0, &kCFTypeSetCallBacks);
        }
...
CFRunLoopTimerRef CFRunLoopTimerCreate(CFAllocatorRef allocator, CFAbsoluteTime fireDate, CFTimeInterval interval, CFOptionFlags flags, CFIndex order, CFRunLoopTimerCallBack callout, CFRunLoopTimerContext *context) {
    ...
    uint64_t now2 = mach_absolute_time();//获取当前相对纳秒
    CFAbsoluteTime now1 = CFAbsoluteTimeGetCurrent();//获取当前绝对触发时间

    if (fireDate < now1) {//触发时间小于当前时间
